...
void atomicKernel_CPU(int *atom_arr, int no_of_threads)
{
    ...
    for (int i=no_of_threads; i<2*no_of_threads; i++)
    {
        ...
        for (int j=0; j < LOOP_NUM; j++)
        {
            ...
            atom_arr[2] = max(atom_arr[2], i);
            __sync_bool_compare_and_swap(&g_lock2, 1, 0);

            // Atomic minimum
            while (!__sync_bool_compare_and_swap(&g_lock2, 0, 1)); 
            atom_arr[3] = min(atom_arr[3], i);
            __sync_bool_compare_and_swap(&g_lock2, 1, 0);

            // Atomic increment (modulo 17+1)
            int limit = 17;
            while (!__sync_bool_compare_and_swap(&g_lock2, 0, 1)); 
            atom_arr[4] = (atom_arr[4] >= limit) ? 0 : atom_arr[4]+1; 
            __sync_bool_compare_and_swap(&g_lock2, 1, 0);
            ...
            limit = 137;
            while (!__sync_bool_compare_and_swap(&g_lock2, 0, 1)); 
            atom_arr[5] = ((atom_arr[5] == 0) || (atom_arr[5] > limit)) ? limit : atom_arr[5]-1;
            __sync_bool_compare_and_swap(&g_lock2, 1, 0);
