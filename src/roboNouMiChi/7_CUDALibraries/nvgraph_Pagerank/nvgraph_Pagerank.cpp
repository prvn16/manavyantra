...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < 2; ++i)
        check_status(nvgraphSetVertexData(handle, graph, vertex_dim[i], i, NVGRAPH_CSC_32));
    check_status(nvgraphSetEdgeData(handle, graph, (void*)weights_h, 0, NVGRAPH_CSC_32));
    
    // First run with default values
    check_status(nvgraphPagerank(handle, graph, 0, alpha1_p, 0, 0, 1, 0.0f, 0));
    
    // Get and print result
    check_status(nvgraphGetVertexData(handle, graph, vertex_dim[1], 1, NVGRAPH_CSC_32));
    ...
    // Get and print result
    check_status(nvgraphGetVertexData(handle, graph, vertex_dim[2], 2, NVGRAPH_CSC_32));
