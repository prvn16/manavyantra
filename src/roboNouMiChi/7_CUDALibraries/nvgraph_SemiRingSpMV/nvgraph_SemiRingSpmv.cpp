...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < vertex_numsets; ++i)
       check_status(nvgraphSetVertexData(handle, graph, vertex_dim[i], i, NVGRAPH_CSR_32));
    check_status(nvgraphAllocateEdgeData  (handle, graph, edge_numsets, &edge_dimT));
    check_status(nvgraphSetEdgeData(handle, graph, (void*)weights_h, 0, NVGRAPH_CSR_32));

    // Solve
    check_status(nvgraphSrSpmv(handle, graph, 0, alpha_p, 0, beta_p, 1, NVGRAPH_PLUS_TIMES_SR));

    //Get and print result
    check_status(nvgraphGetVertexData(handle, graph, (void*)y_h, 1, NVGRAPH_CSR_32));
    printf("y_h\n");
